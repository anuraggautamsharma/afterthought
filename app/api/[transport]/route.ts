...
const handler = createMcpHandler(
  server => {
    ...
    server.tool(
      'create_post',
      'Create a fully-formatted blog post in ONE call. Prefer `blocks` for rich content: an ordered array of { type, ... }. Block types: paragraph {text}, heading {text, level 2|3}, image {url, alt?, ratio?}, video {url} (YouTube/Vimeo), quote {text}, bullet_list {items[]}, ordered_list {items[]}, divider {}, figure {text, figure_style? quote|statement|shapes, theme?, ratio?} — a server-rendered on-brand graphic placed inline (no image work needed; great for pull-quotes and section breaks), and mermaid {text} — a diagram where `text` is Mermaid syntax (flowchart, sequence, architecture, ER, state, timeline, mindmap); it renders as a crisp brand-themed vector diagram, ideal for explaining things visually. Text supports **bold**, *italic*, [label](url). For a COVER: set `auto_cover: true` to instantly render an on-brand hero from the title (recommended; pick `cover_style` blocks|grid|gradient|editorial and `cover_theme` navy|lime|ink), or pass a hosted `cover_image` URL. Body image/video urls must be publicly hosted (use upload_image_from_url or list_media). `faqs` adds an FAQ section + FAQPage schema. status "published" makes it live; default "draft". If you only have plain text, pass `body` instead of `blocks`.',
      {
        title: z.string(),
        blocks: z.array(z.object({
          type: z.enum(['paragraph', 'heading', 'image', 'video', 'quote', 'bullet_list', 'ordered_list', 'divider', 'figure', 'mermaid']),
