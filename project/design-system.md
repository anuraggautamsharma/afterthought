# Afterthought Design System

## Overview

Afterthought's marketing canvas is, at the system level, an editor-clean black-and-white frame. The chrome — top nav, body type, footer, primary CTA — is monochrome. Headlines are oversized `{typography.display-xl}` set in `afterthoughtSans` with aggressive negative tracking, body copy hovers around weight 320–340 of the same variable family, and small mono `{typography.eyebrow}` and `{typography.caption}` labels (afterthoughtMono, all-caps, positive tracking) act as section markers. Every CTA is a pill — `{rounded.pill}` — and the primary action across the entire site is the same black `{components.button-primary}` paired with the same white `{components.button-secondary}`.

What makes the design unique is what happens **between** those monochrome bookends: the page repeatedly drops into oversized pastel **color-block sections** — lime, lavender, cream, mint, pink, coral, and a deep navy — that span the full content width with `{rounded.lg}` corners and `{spacing.xxl}` interior padding. These blocks are where the storytelling lives. They aren't accents tucked into a card; they take over a whole viewport's worth of vertical space, like a designer arranging giant sticky notes on a clean wall. The collaboration board is the most pastel-saturated, the home page rotates through the full set, and the pricing page ends with a lime FAQ panel — same vocabulary, different rhythm per route.
...
- `afterthoughtSans` variable typeface used at unusually fine weight increments (320, 330, 340, 450, 480, 540) — the type system reads as a single voice that flexes rather than a multi-weight family.
- Tight negative letter-spacing on display sizes (-1.72px at 86px, -0.96px at 64px) creates a confident editorial cadence.
- `afterthoughtMono` reserved for category labels, eyebrows, and captions — always uppercase, positive tracking — to flag taxonomy without competing with display type.
...
> Source pages: home, /design/, /collaboration-board/brainstorming-tool/, /pricing/, /contact/.
...
- **Block Cream** ({colors.block-cream}): Soft warm background — collaboration-board hero strip, template-grid section.
- **Block Mint** ({colors.block-mint}): Collaboration-board pastel section.
- **Block Pink** ({colors.block-pink}): Collaboration-board pastel section.
...
- **afterthoughtSans** — Afterthought's proprietary variable typeface; fallback stack `afterthoughtSans Fallback, SF Pro Display, system-ui, helvetica`. Variable weight axis is exercised at unusually fine increments (320, 330, 340, 450, 480, 540, 700) — the design system reads as a single voice modulating rather than a stepped weight family.
- **afterthoughtMono** — Proprietary monospace; fallback `afterthoughtMono Fallback, SF Mono, menlo`. Used exclusively for eyebrow labels and captions, always uppercase with positive letter-spacing.
...
|---|---|---|---|---|---|
| `{typography.display-xl}` | 86px | 340 | 1.00 | -1.72px | Hero headlines (home, collaboration board) |
...
| `{typography.eyebrow}` | 18px | 400 | 1.30 | 0.54px | afterthoughtMono uppercase section eyebrows |
| `{typography.caption}` | 12px | 400 | 1.00 | 0.60px | afterthoughtMono uppercase captions, footer column heads |
...
- **Mono is taxonomy, not body.** afterthoughtMono is reserved for eyebrows and captions — never used to set a paragraph.
...
If implementing without access to afterthoughtSans / afterthoughtMono, suitable open-source substitutes are **Inter** (or **Geist**) for the sans, and **JetBrains Mono** (or **Geist Mono**) for the mono. Inter at variable weights closely matches the fine-grained weight axis afterthoughtSans uses; expect to manually adjust line-heights down by ~0.02 to compensate for Inter's slightly taller x-height.
...
- Universal rhythm constant: `{spacing.section}` (96px) — the vertical gap between major content sections holds across home, pricing, and collaboration-board pages.
...
- Three- and four-column grids on the desktop pricing comparison and collaboration-board template galleries.
...
Afterthought's marketing system is shadow-light by design — the color blocks substitute for traditional elevation. Where most SaaS sites use a shadowed white card to draw attention, Afterthought uses a saturated background panel. This makes the rare actual shadow (e.g., a floating template card hovering over a cream section) feel like an exception worth noticing.
...
- **Sticky-note style component thumbnails** in the collaboration board — slightly off-axis pastel rectangles arranged like notes on a board — read as collage, not card-stack.
- **Embedded product UI mocks** (design panels, board canvas snippets) appear as flat compositions on color blocks; their internal shadows are subtle and stay within the mock.
...
- Collaboration-board pastel sticky-note component thumbnails preserve a small `{rounded.sm}` corner that mimics actual sticky paper.
- No avatar circles appear in marketing surfaces — Afterthought's marketing avoids personification.
...
**`button-magenta-promo`** — Saturated pink pill used only inside promotional surfaces such as the lilac "Save your spot" Release Notes banner. Reserved for moments where Afterthought's product team wants the CTA to pop against an already-colored panel.
...
**`template-card`** — Thumbnail tile in the home "Explore what people are making" grid and the collaboration-board template gallery.
...
The defining surface of Afterthought's marketing. Each is a full-content-width panel with `{rounded.lg}` corners and `{spacing.xxl}` interior padding. Variants:
...
**`color-block-section-lilac`** — lavender ground for `/design/` hero and collaboration-board highlight sections.
...
**`footer`** — Dense link grid on white canvas with the wordmark "Afterthought" set in display weight at the top-left.
...
- Keep type in `afterthoughtSans` at variable weights — pick from 320, 330, 340, 480, 540, 700 to express hierarchy. Avoid intermediate weights outside this set.
- Use `afterthoughtMono` only for eyebrows and captions, always uppercase, with the documented positive letter-spacing.
...
- Don't introduce mid-gray text. Body hierarchy comes from `afterthoughtSans` weight, not from opacity.
...
- Don't combine more than one color block visible inside a single viewport — Afterthought's pacing always lets the white canvas separate them.
- Don't square off CTAs. Square buttons read as a different brand.
- Don't put `afterthoughtMono` in body copy — it's a taxonomy tool, not a reading typeface.
...
- Sticky-note style collaboration-board thumbnails maintain their slight off-axis rotation across breakpoints — the rotation is a brand signal, not a desktop-only flourish.
